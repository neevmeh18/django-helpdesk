...
class FollowUp(models.Model):
    ...
    #: A public follow-up written by staff, so it goes out to the submitter.
    DIRECTION_OUTBOUND = "outbound"
    #: A public follow-up that came in from the submitter.
    DIRECTION_INBOUND = "inbound"

    @property
    def is_from_submitter(self):
        """Whether this follow-up was written outside the helpdesk.

        A follow-up that arrived by e-mail carries the submitter's message ID.
        Otherwise we go by the author: a non-staff user is treated as the
        submitter, and a staff user or no user at all is not. This
        misclassifies one case: the public ticket view does not currently
        record a user for anonymous submissions, so those follow-ups end up
        with no user and are indistinguishable from ones generated by the
        helpdesk itself -- reported here as not from the submitter.
        """
        ...
        if self.user is None:
            return False
        return not is_helpdesk_staff(self.user)

    @property
    def direction(self):
        """Which way this follow-up traveled, for display purposes.

        Private follow-ups stay inside the helpdesk, so they are always
        ``DIRECTION_INTERNAL`` regardless of who wrote them.
        """
        if not self.public:
            return self.DIRECTION_INTERNAL
        if self.is_from_submitter:
